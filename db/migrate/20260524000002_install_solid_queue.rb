...
# Installs Solid Queue tables into the primary database. We use a single-DB
# setup (no separate queue: connection) because this app does not configure
# config.solid_queue.connects_to in production. This matches what the
# SolidQueue::InstallGenerator copies into db/queue_schema.rb, expressed as
# a Rails migration so it runs through the normal db:migrate flow.
