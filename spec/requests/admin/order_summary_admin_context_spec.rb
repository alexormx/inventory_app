...
RSpec.describe 'Order summary layout context', type: :request do
  ...
  context 'when accessed from admin namespace summary' do
    it 'renders using admin layout (sidebar present)' do
      sign_in admin
      get summary_admin_sale_order_path(order), headers: { 'ACCEPT' => 'text/html' }
  ...
  context 'when accessed by customer (OrdersController summary)' do
