...
module Admin
  class UsersController < ApplicationController
    ...
    def index
      ...
      users = users.where(Arel.sql("#{sales_total_expr} > 0")) if @with_sales
      users = users.where(Arel.sql("#{purchases_total_expr} > 0")) if @with_purchases
      users = users.where(credit_enabled: true) if @with_credit
      if @active_recent
      ...
      users = if sort_map.key?(sort)
                users.reorder(Arel.sql("#{sort_map[sort]} #{dir.upcase}"))
              else
                users.reorder(created_at: :desc)
              end
      ...
      filtered_counts_scope = filtered_counts_scope.where(role: @role_filter) if @role_filter.present? && @role_filter != 'all'
      # Replicar filtros agregados para que los contadores reflejen la vista
      filtered_counts_scope = filtered_counts_scope.where(Arel.sql("#{sales_total_expr} > 0")) if @with_sales
      filtered_counts_scope = filtered_counts_scope.where(Arel.sql("#{purchases_total_expr} > 0")) if @with_purchases
      filtered_counts_scope = filtered_counts_scope.where(credit_enabled: true) if @with_credit
      if @active_recent
        active_sql = User.sanitize_sql_array(["(#{last_visit_expr}) IS NOT NULL AND (#{last_visit_expr}) >= ?", 30.days.ago])
    ...
    def user_params
      params.expect(
        user: [:name, :email, :phone, :role, :discount_rate, :created_offline,
