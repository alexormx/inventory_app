class Admin::InventoryAdjustmentsController < ApplicationController
	before_action :set_inventory_adjustment, only: [:show, :edit, :update, :destroy, :apply, :reverse]
	...
	def inventory_adjustment_params
		params.require(:inventory_adjustment).permit(
			:status, :adjustment_type, :found_at, :reference, :note, :user_id,
