  ...
  class DashboardController < ApplicationController
    ...
    def index
      ...
      top_products = SaleOrderItem.joins(:sale_order, :product)
                                  ...
                                  .order('units DESC')
                                  .limit(10)
      @top_products_all = top_products.map do |r|
        { product_id: r.id, name: r.product_name, brand: r.brand, category: r.category, units: r.attributes['units'].to_i, 
      ...
      if last20_ids.any?
        top_last20 = SaleOrderItem.joins(:product)
                                  ...
                                  .order('units DESC')
                                  .limit(5)
        @top_products_last20 = top_last20.map do |r|
          { product_id: r.id, name: r.product_name, units: r.attributes['units'].to_i, revenue: r.attributes['revenue'].to_d }
        end
      else
        @top_products_last20 = []
      end
      ...
      users_top = SaleOrderItem.joins(sale_order: :user)
                               ...
                               .order('revenue DESC')
                               .limit(10)
      @top_users_all = users_top.map do |r|
        { user_id: r.id, name: r.name.presence || r.id, orders_count: r.attributes['orders_count'].to_i, revenue: r.attributes['revenue'].to_d }
      end
      ...
      users_top_range = SaleOrderItem.joins(sale_order: :user)
                                     ...
                                     .order('revenue DESC')
                                     .limit(10)
      @top_users_range = users_top_range.map do |r|
        { user_id: r.id, name: r.name.presence || r.id, orders_count: r.attributes['orders_count'].to_i, revenue: r.attributes['revenue'].to_d }
      end
      ...
      users_top_last_year = SaleOrderItem.joins(sale_order: :user)
                                         ...
                                         .order('revenue DESC')
                                         .limit(10)
      @top_users_last_year = users_top_last_year.map do |r|
        { user_id: r.id, name: r.name.presence || r.id, orders_count: r.attributes['orders_count'].to_i, revenue: r.attributes['revenue'].to_d }
      end
      ...
      def sellers
        ...
        rel = build_rel.call(fallback_scope).limit(10) unless SaleOrderItem.joins(:sale_order).merge(primary_scope).exists?
      ...
      def customers_rank
        ...
        rows = case metric
               when 'reserved'
                 reserved_rows.map do |r|
                   { user_id: r.attributes['user_id'].to_i, name: r.attributes['name'].presence || r.attributes['user_id'], units_reserved: r.attributes['units_reserved'].to_i, 
               ...
               when 'combined'
                 ...
                 sales_rows_paid.map do |r|
                   { user_id: r.attributes['user_id'].to_i, name: r.attributes['name'].presence || r.attributes['user_id'], orders_count: r.attributes['orders_count'].to_i, 
                     revenue: r.attributes['revenue'].to_d }
                 end
                                .sort_by { |h| -h[:revenue] }
               end
      ...
      top_sellers_q = SaleOrderItem.joins(:sale_order, :product)
                                   ...
                                   .order('units DESC')
                                   .limit(10)
      @top_sellers_ytd = top_sellers_q.map do |r|
        { product_id: r.id, name: r.product_name, brand: r.brand, category: r.category, units: r.attributes['units'].to_i, 
          revenue: r.attributes['revenue'].to_d }
      end
      ...
      top_sellers_ly_q = SaleOrderItem.joins(:sale_order, :product)
                                      ...
                                      .order('units DESC')
                                      .limit(10)
      @top_sellers_last_year = top_sellers_ly_q.map do |r|
        { product_id: r.id, name: r.product_name, brand: r.brand, category: r.category, units: r.attributes['units'].to_i, 
          revenue: r.attributes['revenue'].to_d }
      end
      ...
      inv_only_statuses   = %i[available in_transit]
      reserved_statuses   = %i[reserved pre_reserved pre_sold]
      inv_all_statuses    = inv_only_statuses + reserved_statuses

      build_top_inv = lambda do |statuses, _order_by|
        base = Inventory.joins(:product)
                        .where(status: statuses)
                        .group('products.id', 'products.product_name', 'products.brand', 'products.category')
        rows_v = base.select('products.id, products.product_name, products.brand, products.category, SUM(inventories.purchase_cost) AS inventory_value')
                     .order('inventory_value DESC')
                     .limit(10)
        by_value = rows_v.map do |r|
          { product_id: r.id, name: r.product_name, brand: r.brand, category: r.category, inventory_value: r.attributes['inventory_value'].to_d }
        end
        ...
        rows_q = base.select('products.id, products.product_name, products.brand, products.category, COUNT(inventories.id) AS units_count, SUM(inventories.purchase_cost) AS inventory_value')
                     .order('units_count DESC')
                     .limit(10)
        by_qty = rows_q.map do |r|
          { product_id: r.id, name: r.product_name, brand: r.brand, category: r.category, units_count: r.attributes['units_count'].to_i, 
      ...
      waste_rows_ytd = Inventory.joins(:product)
                                .where(status: %i[marketing damaged lost scrap], status_changed_at: range)
                                .group('products.id', 'products.product_name', 'products.brand', 'products.category')
      ...
      waste_rows_ly = Inventory.joins(:product)
                               .where(status: %i[marketing damaged lost scrap], status_changed_at: ly_start..ly_end)
                               .group('products.id', 'products.product_name', 'products.brand', 'products.category')
                               .select('products.id AS product_id, products.product_name AS product_name, products.brand AS brand, products.category AS category, SUM(inventories.purchase_cost) AS waste_total')
      ...
      waste_rows_all = Inventory.joins(:product)
                                .where(status: %i[marketing damaged lost scrap])
                                .group('products.id', 'products.product_name', 'products.brand', 'products.category')
                                .select('products.id AS product_id, products.product_name AS product_name, products.brand AS brand, products.category AS category, SUM(inventories.purchase_cost) AS waste_total')
      ...
      # ============== Worst Products KPIs ==================
      # Inputs per product
      inv_stock_map    = Inventory.where(status: %i[available in_transit]).group(:product_id).count
      inv_reserved_map = Inventory.where(status: %i[reserved pre_reserved pre_sold]).group(:product_id).count
      ...
      products_info = Product.where(id: product_ids)
                             ...
                             .to_h { |id, name, brand, category, sp, apc| [id, { name:, brand:, category:, sp: sp.to_d, apc: apc.to_d }] }
      ...
      product_ids.each do |pid|
        ...
        doh = if avg_monthly_sales.positive?
                (stock_q.to_f * 30.0) / avg_monthly_sales
              else
                (stock_q.positive? ? 1.0 / 0 : 0.0)
              end
      ...
      margin_vals = per_product.pluck(:margin_pct).compact
      rot_vals    = per_product.pluck(:rotation).compact
      doh_vals    = per_product.pluck(:doh).select(&:finite?)
      ...
      margins = per_product.pluck(:margin_pct)
      rotations = per_product.pluck(:rotation)
      dohs     = per_product.pluck(:doh)
      caps     = per_product.map { |h| h[:immobilized_capital].to_f }

      # Worst lists
      @worst_margin_products = per_product.select { |h| h[:units_sold].to_i.positive? && h[:margin_pct].present? }
    ...
    def detect_country_in(norm_text)
      ...
      if norm_text.include?('estados unidos') || norm_text.include?('eeuu') || norm_text.include?('ee. uu') || norm_text.include?('usa') || norm_text.include?('united states')
