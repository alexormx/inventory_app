...
function buildBlankPurchaseOrderItemRow(index) {
  ...
  // TODO: Remove the fields according to context
  // --- Quantity ---
...
function buildInputTd(field, index, options = {}) {
  ...
  if (field) {
    input.name = `order[order_items_attributes][${index}][${field}]`;
  }
...
function updateTotals() {
  ...
  if(context === "sale-order") {
    ...
    const taxTotal = document.querySelector("#sale_order_tax_total");
    if (taxRate && taxTotal) {
