...
export default class extends Controller {
  ...
  connect(){
    this._timer = null
  if(window.APP_DEBUG) console.log('[product-search] connect', { url: this.urlValue, minLength: this.minLengthValue })
  ...
  if(window.APP_DEBUG) console.log('[product-search] attached fallback listeners')
    }
    // Sentinel flags for troubleshooting
    this.element.dataset.productSearchAttached = '1'
    window.__ProductSearchConnectCount = (window.__ProductSearchConnectCount || 0) + 1
  }

  input(){
    const q = this.inputTarget.value.trim()
  if(window.APP_DEBUG) console.log('[product-search] input handler', { raw: this.inputTarget.value, trimmed: q, len: q.length, minLength: this.minLengthValue })
  ...
  if(window.APP_DEBUG) console.log('[product-search] fetching', url)
  ...
  render(products){
  if(window.APP_DEBUG) console.log('[product-search] render', { count: Array.isArray(products) ? products.length : 'invalid' })
