...
module InventoryReconciliation
  # Revisa consistencia entre Inventory y PurchaseOrderItem
  # - Orphans: inventory con purchase_order_id pero sin purchase_order_item_id o cuyo POI desapareció => se destruyen
  # - Missing: purchase_order_items cuya cantidad > piezas existentes => se crean piezas faltantes
  # Sólo actúa sobre productos/POs existentes; ignora sale order links.
  class ReconcilePurchaseOrderLinksService
    ...
    # Inventario con purchase_order_id pero sin POI válido
    def destroy_orphans
      ...
      orphan_scope.limit(@limit).find_each do |inv|
        ...
        begin
          inv.destroy!
          InventoryEvent.create!(inventory_id: inv.id, product_id: inv.product_id, event_type: 'reconciliation_orphan_destroyed', metadata: { purchase_order_id: inv.purchase_order_id }) rescue nil
        rescue => e
    ...
    # Para cada POI, asegurar que existan piezas = quantity
    def create_missing
      created = 0
      PurchaseOrderItem.find_each do |poi|
        ...
        needed.times do
          ...
          begin
            ...
            InventoryEvent.create!(inventory_id: ::Inventory.last.id, product_id: poi.product_id, event_type: 'reconciliation_missing_created', metadata: { purchase_order_id: poi.purchase_order_id, purchase_order_item_id: poi.id }) rescue nil
          rescue => e
