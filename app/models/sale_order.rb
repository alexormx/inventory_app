...
class SaleOrder < ApplicationRecord
  ...
  scope :ordered_due_date_recent, lambda {
  ...
  def ensure_payment_and_shipment_present
    ...
    when 'In Transit'
      ...
      errors.add(:shipment, 'must exist to set in transit') if shipment.blank?
    when 'Delivered'
      # Si no hay crédito, exigir pago; si hay crédito, permitir sin pago
      errors.add(:payment, 'must exist to deliver the order') if !credit_allowed? && !(total_order_value.to_f == 0.0 || payments.any?)
      errors.add(:shipment, 'must exist to deliver the order') if shipment.blank?
    end
  end
  ...
  def ensure_shipment_status_matches
    ...
    if shipment.present?
      ...
      begin
        order_base = order_date || Time.zone.today
        est = order_base + 20
        create_shipment!(
    ...
    return unless credit_allowed? && due_date.blank?

    base = shipment&.actual_delivery || shipment&.estimated_delivery || order_date || Time.zone.today
