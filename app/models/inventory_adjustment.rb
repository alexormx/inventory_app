class InventoryAdjustment < ApplicationRecord
	...
	def generate_reference_if_needed!(now = Time.current)
		...
		pattern = SystemVariable.get('INVENTORY_ADJ_REFERENCE_PATTERN', 'ADJ-YYYYMM') rescue 'ADJ-YYYYMM'
		...
		last = self.class.where("reference LIKE ?", "#{prefix}-%").where.not(reference: nil)
				.order(Arel.sql("reference DESC")).limit(1).pluck(:reference).first
		seq = if last
			last.split('-').last.to_i + 1
		else
			1
		end
		self.reference = sprintf("%s-%02d", prefix, seq)
	end
